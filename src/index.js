...
class ReportTool extends React.Component {
  ...
  processSubmitLoadAction() {
    ...
    let urlLoad = generateFmsJsonUrl(this.state.descriptionFileObj1.s3Path, this.state.mod);
...
//               if (matchCount > doneCount) { break; }			// to process entries only until desired amount, Ranjana wants to always process everything to get counts
  ...
  handleSubmitGenerateLink(event) {
    ...
    if (this.state.mod === undefined) { errorMessage += 'Choose a mod\n';  }
    if (errorMessage !== '') { alert(errorMessage); return; }
    let urlDownload = generateFmsJsonUrl(this.state.descriptionFileObj1.s3Path, this.state.mod);
    ...
    let filename = this.state.descriptionFileObj1.uploadDate + '_' + this.state.mod + '.json';
  ...
  handleSubmitCompare(event) {
    ...
    if (this.state.mod === undefined) {       errorMessage += 'Choose a mod\n';               }
    if (errorMessage !== '') { alert(errorMessage); return; }
    let url1 = generateFmsJsonUrl(this.state.descriptionFileObj1.s3Path, this.state.mod);
    ...
    let url2 = generateFmsJsonUrl(this.state.descriptionFileObj2.s3Path, this.state.mod);
