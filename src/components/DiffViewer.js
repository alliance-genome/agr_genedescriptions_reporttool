...
import {
    getDiffFilterGeneName, getSelectedDiffField, getDiffFilterGeneNameCS, getDiffFilterGeneNameSubstr,
    getDiffFilterPhrase, getDiffFilterPhraseCS, getFilesContent, getSelectedFilesInfo, getSelectedMod
...
const DiffViewer = (props) => {
    ...
    useEffect(() => {
        if (props.filesContent[0] !== undefined && props.filesContent[1] !== undefined) {
            processDataCompare(props.filesContent[0], props.filesContent[1]);
            setIsLoading(false);
        } else {
            if (props.selectedFilesInfo[0] !== undefined && props.selectedFilesInfo[1] !== undefined) {
                setIsLoading(true);
    ...
    const processDataCompare = async (response1, response2) => {
        ...
        if (props.diffFilterPhrase !== '') {
            setTextDivDiffResults(textDivDiffResults.concat("Filtering on keyword " + props.diffFilterPhrase + "\n\n"));
        }
        else {
